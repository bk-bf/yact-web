    ...
    const fetchData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Dispatch sync event so markets/watchlist page bodies stay in
          // sync with the header — mirrors what AppShellLayout does.
          window.dispatchEvent(
            new CustomEvent("yact:markets-sync", { detail: data }),
          );
        }
      } catch {
...
>
  <TuiTopbar {globalData} {coinCount} {clockTime} {blinkOn} {activePage} />
