...
function getHighlights(
  coins: Array<{ priceChangePercentage24h: number; totalVolume24h: number }>,
) {
...
export async function GET({ fetch }) {
  ...
  try {
    ...
    clearTimeout(timer);
    return json(
      { error: "markets upstream timed out or unreachable" },
      { status: 503 },
    );
  } finally {
    clearTimeout(timer);
  ...
  const snapshotTs =
    typeof marketsPayload?.ts === "number" ? marketsPayload.ts : null;
  return json({
    ...marketsPayload,
    snapshotTs,
