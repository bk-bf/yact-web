<script lang="ts">
  ...
  $effect(() => {
    ...
    let cancelled = false;
    fetch(`/api/coins/${encodeURIComponent(coinId)}/regime-match`, {
      cache: "no-store",
    })
      .then(async (res) => {
        if (cancelled) return;
      ...
      .catch((err: unknown) => {
        if (cancelled) return;
        fetchError =
          err instanceof Error ? err.message : "Failed to load regime match";
      })
      .finally(() => {
...
<CoinRailCard title="Regime Match">
  ...
  {:else if !data?.regime}
    <p class="regime-empty">
      ...
      <a href="/currencies/{coinId}/analytics" class="regime-link"
        >Run a sweep →</a
      >
    </p>
  {:else}
    ...
    <div class="regime-match">
      <div class="regime-score-row">
        ...
        <span
          class="regime-score-value"
          style="color: {matchScoreColor(r.match_score)}"
        >
      ...
      <div class="regime-stats">
        <div class="regime-stat">
          ...
          <strong
            class="regime-stat__value"
            class:positive={r.sharpe_ratio >= 1}
        ...
        <div class="regime-stat">
          ...
          <strong
            class="regime-stat__value"
            class:positive={r.total_return_pct >= 0}
      ...
      <a
        href="/currencies/{coinId}/analytics"
        class="regime-link regime-link--footer"
      >
        Full sweep →
      </a>
    </div>
