<script lang="ts">
  ...
  import { loadWatchlistPageData, createInitialWatchlistPageData } from "../watchlist/watchlist-page.data";
  import {
    getMarketsDataCache,
    setMarketsDataCache,
  ...
  import { smartUsd, percent } from "../../utils/formatters";
  import {
    sparklinePath,
    chartDirectionClass,
    SPARKLINE_WIDTH,
  ...
  const watchlistIds = getContext<WatchlistIdsContext>(WATCHLIST_IDS_CONTEXT_KEY);
  const progressive = useProgressiveDataLoad(() => createInitialWatchlistPageData());
      ...
      <div class="watchlist-suggestions">
        {#each suggestedCoins as coin (coin.id)}
          ...
          <button
            ...
            aria-label="{alreadyAdded ? 'Remove' : 'Add'} {coin.name} {alreadyAdded
              ? 'from'
              : 'to'} watchlist"
          >
