<script lang="ts">
	...
	const signalLog: SignalEntry[] = [
		{ label: 'LAG', detail: 'log 16s — strong — coinbase +$61 · binance +$64', tag: 'SKIP' },
		{ label: 'LAG', detail: 'spent 2 weeks watching order book — not trading', tag: 'WATCH' },
		{ label: 'LAG', detail: '14 0s gap — binance +$551 · coinbase +$50', tag: 'GAP' },
		{ label: 'LAG', detail: '14 2s gap detected — chainlink not updated yet', tag: 'PENDING' },
		{ label: 'KILL', detail: '6.2s — too short — noise — skip this one', tag: 'SKIP' },
		{ label: 'LAG', detail: 'spent 2 weeks watching order book — not trading', tag: 'WATCH' },
		{ label: 'KILL', detail: '9.1s — below 11s threshold — skip', tag: 'SKIP' },
		{ label: 'LAG', detail: '14 7s gap detected — chainlink not updated yet', tag: 'PENDING' },
		{ label: 'LAG', detail: 'spent 2 weeks watching order book — not trading', tag: 'WATCH' },
		{ label: 'ENTER', detail: 'both signals agree · BTC 71c — entering', tag: 'MATCH' },
		{ label: 'EXIT', detail: 'wrong direction — $10 — cut immediately', tag: 'CUT' },
		{ label: 'KILL', detail: 'after 100s — no exit window — if wrong', tag: 'TIMEOUT' },
		{ label: 'ENTER', detail: 'tag 11.0s — imbalance 2.1 — entering', tag: 'MATCH' },
		{ label: 'EXIT', detail: 'imbalance 0.48 — sellers dominant — skip', tag: 'SKIP' },
		{ label: 'KILL', detail: 'after 100s — no exit window — if wrong', tag: 'TIMEOUT' },
		{ label: 'SCAN', detail: 'market opens — bot watches — does NOT enter', tag: 'WATCH' },
		{ label: 'ENTER', detail: 'tag 13.0s + imbalance 2.01 — entering at 51c', tag: 'MATCH' },
		{ label: 'EXIT', detail: 'both signals agree — BTC 71c — entering', tag: 'ENTER' },
	];

	const imbalanceLog: ImbalanceEntry[] = [
		{ key: 'RATIO', value: '2.08', detail: '30s window — both signals agree', action: 'ENTER' },
		{ key: 'RATIO', value: '2.08', detail: '10s window — both signals agree', action: 'SKIP' },
		{ key: 'BID DEPTH', value: '$3,848', detail: 'ask depth $998 — ratio 1.04', action: null },
		{ key: 'IMBALANCE', value: '0.44+0.55', detail: 'sellers dominant', action: 'SKIP' },
		{
			key: 'BID DEPTH',
			value: '$3,848',
		...
		{ key: 'IMBALANCE', value: '0.44+0.55', detail: 'sellers dominant', action: 'SKIP' },
		{ key: 'RATIO', value: '2.08', detail: '10s window — BOTH SIGNALS', action: 'ENTER' },
		{ key: 'RATIO', value: '1.61', detail: 'lag 14s — window 80s', action: 'ENTER' },
		{ key: 'RATIO', value: '2.08', detail: '10s window — both signals agree', action: 'SKIP' },
		{ key: 'IMBALANCE', value: '1.2', detail: 'neutral zone — skip this window', action: 'SKIP' },
		{
			key: 'IMBALANCE',
			value: '1.84',
	...
	const positions: Position[] = [
		{ pair: 'BTC 5m', price: '$74,795', pnlAbs: '+$18.8k', pnlPct: '+25.1%', status: 'profit' },
		{ pair: 'BTC 5m', price: '$74,770', pnlAbs: '+$2.1k', pnlPct: '+26.1%', status: 'profit' },
		{ pair: 'BTC 5m', price: '$74,635', pnlAbs: '+$1.6k', pnlPct: '+20.8%', status: 'profit' },
		{ pair: 'BTC 5m', price: '$74,830', pnlAbs: '-$1.31', pnlPct: '-1.7%', status: 'loss' },
		{ pair: 'BTC 5m', price: '$74,825', pnlAbs: '+$0.9k', pnlPct: '+12.6%', status: 'profit' },
		{ pair: 'BTC 5m', price: '$74,775', pnlAbs: '+$0.6k', pnlPct: '+8.9%', status: 'profit' },
	];

	const signalTree = [
		{ indent: 0, label: 'EXIT', detail: 'wrong direction — $10 — cut immediately', tag: 'CUT' },
		{ indent: 0, label: 'KILL', detail: 'after 100s — no exit window if wrong', tag: 'TIMEOUT' },
		{
			indent: 0,
			label: 'KILL',
		...
		{ indent: 0, label: 'ENTER', detail: 'tag 11.0s — imbalance 2.1 — entering', tag: 'ENTER' },
		{ indent: 1, label: 'EXIT', detail: 'imbalance 0.48 — sellers dominant', tag: 'SKIP' },
		{
			indent: 0,
			label: 'KILL',
		...
		{ indent: 0, label: 'ENTER', detail: 'entry window 7c — $74,825 BTC 71c — in', tag: 'MATCH' },
		{ indent: 1, label: 'EXIT', detail: 'both signals agree — BTC 71c — entering', tag: 'ENTER' },
		{
			indent: 0,
			label: 'KILL',
	...
	// ── PnL sparkline data ─────────────────────────────────────────────────────
	const pnlData = [
		220, 380, 310, 460, 420, 530, 500, 640, 610, 720, 700, 790, 760, 860, 840, 940, 920, 1020, 1000,
		1100, 1080, 1200, 1180, 1300, 1280, 1420, 1400, 1540, 1520, 1660, 1640, 1760, 1740, 1880, 1920,
		2020, 2000, 2140, 2120, 2240, 2220, 2360, 2420, 2400, 2540, 2520, 2660, 2640, 2780, 2840, 2820,
		2960, 2940, 3080, 3160, 3240, 3320, 3400, 3520, 3640, 3780, 3920, 4080, 4220, 4360, 4500, 4660,
		4800, 4960, 5120, 5280, 5440, 5600, 5760, 5940, 6100, 6280, 6460, 6640, 6800, 6980, 7180, 7360,
		7540, 7740, 7940, 8140, 8360, 8580, 8820, 9080, 9360, 9620, 9900, 10180, 10460, 10760, 11060,
		11356,
	];
...
<div class="terminal-root">
	...
	<div class="chart-band">
		<div class="chart-label-tl">
			...
			<span class="chart-sub">Claude code · Beta BTC windows · 1,847 trades · Total 12.4m</span>
		</div>
			...
			<div class="log-feed">
				{#each signalLog as entry}
					<div class="log-row">
						...
						<span class="log-tag" style="color:{tagColor(entry.tag)}">{entry.tag}</span>
					</div>
				{/each}
			</div>
		</div>
			...
			<div class="signal-tree">
				{#each signalTree as node}
					<div class="tree-row" style="padding-left:{node.indent * 12 + 4}px">
						<span class="tree-label" style="color:{labelColor(node.label)}">{node.label}</span>
						<span class="tree-detail">{node.detail}</span>
						<span class="tree-tag" style="color:{tagColor(node.tag as SignalTag)}"
							>{node.tag}</span
						>
					</div>
				{/each}
	...
	<div class="bottom-row">
		...
		<div class="panel panel-signals-a">
			<div class="panel-header">
				<span class="panel-dot" style="background: var(--tv-highlight-soft)"></span>
				SIGNAL ANALYSIS
			...
			<div class="signal-bars">
				{#each signalBarsA as bar}
					<div class="signal-bar-row">
						...
						<span class="sbar-pct" style="color:{barColor(bar.pct)}">{bar.pct}%</span>
					</div>
				{/each}
			</div>
		</div>
		...
		<div class="panel panel-heatmap">
			...
			<div class="heatmap-grid">
				{#each [0.540, 0.735, 0.708, 0.775, 0.740, 0.755, 0.720, 0.700, 0.475] as v, i}
					<div
						class="heatmap-cell"
						style="opacity:{0.3 + v * 0.7}; background:{v > 0.72 ? 'rgba(29,223,114,0.6)' : v > 0.6 ? 'rgba(176,38,255,0.5)' : 'rgba(255,77,87,0.5)'}"
					>
						{v.toFixed(3)}
					</div>
				{/each}
