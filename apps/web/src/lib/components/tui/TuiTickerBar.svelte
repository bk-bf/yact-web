...
<div class="t-ticker-bar">
  ...
  {#if coins.length > 0}
    <div class="ticker-overflow" aria-live="off" aria-label="Top coins by market cap">
      <div class="ticker-inner" style="animation-duration: {coinDur}s">
        {#each [...coins, ...coins] as coin, i (i)}
          <span class="ticker-item">
            ...
            >{coin.priceChangePercentage24h >= 0 ? "▲" : "▼"}{Math.abs(coin.priceChangePercentage24h).toFixed(2)}%</span>
          </span>
...
<style>
  ...
  .ti-sym   { color: #edf5f1; font-weight: 600; font-size: 0.68rem; }
  .ti-price { color: rgba(200, 212, 207, 0.8); font-variant-numeric: tabular-nums; }
  .ti-chg   { font-size: 0.63rem; }
  .ticker-sep       { color: rgba(176, 38, 255, 0.3); padding: 0 0.15rem; flex-shrink: 0; }
  .ticker-placeholder { padding: 0 0.75rem; color: rgba(200, 212, 207, 0.3); font-size: 0.65rem; }
  .pos { color: #1ddf72; }
  .neg { color: #ff4d57; }
