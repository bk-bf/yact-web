...
| 2026-04-18 | `apps/web/tsconfig.json`                | `Cannot read file '.../.svelte-kit/tsconfig.json'`                         | `start-features.sh` deletes `.svelte-kit/` on every start and recreates it via `svelte-kit sync`; VS Code TS server reads tsconfig before sync completes. Resolves automatically once the dev server is up. | Wait ~5 s after `start-features.sh start` completes, or run `npx svelte-kit sync` in `apps/web/` |
