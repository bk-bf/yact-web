...
| 2026-04-18 | All `.svelte` files in `features/quant-pipeline` worktree | `Cannot find module '$app/environment'`, `interface is reserved`, `Cannot find name '$props'/$state/$effect/$derived` | `.svelte-kit/tsconfig.json` not yet generated in the worktree — VS Code extension falls back to plain-JS parsing. `svelte-check` after `svelte-kit sync` confirms 0 errors.                                 | Run `pnpm install && npx svelte-kit sync` once in `apps/web/` of the worktree, or start the dev server via `start-features.sh` |
| 2026-04-18 | `features/quant-pipeline/apps/web/tsconfig.json`          | `Cannot read file '.../.svelte-kit/tsconfig.json'`                                                                    | `start-features.sh` deletes `.svelte-kit/` on every start and recreates it via `svelte-kit sync`; VS Code TS server reads tsconfig before sync completes. Resolves automatically once the dev server is up. | Wait ~5 s after `start-features.sh start` completes, or run `npx svelte-kit sync` in `apps/web/` of the worktree               |
