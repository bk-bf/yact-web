...
- Dev servers (main + feature branches) are managed by **dashy** at `http://localhost:7800` — start/stop/restart from the dashboard or via the API.
...
After creating a new worktree and running `sync-workspace.sh`, create a `dashy.json` in the worktree root (copy the pattern from any existing `features/*/dashy.json`, update `id`, `worktree`, `port`, `pid_file`, and `cwd`). Dashy auto-discovers it within `scan_interval_sec`.
...
| Branch         | Port | Notes                                                           |
| -------------- | ---- | --------------------------------------------------------------- |
| `visual-style` | 5177 | Docked into dashy; previously excluded from `start-features.sh` |

## Known LSP false positives

| Date       | File(s)                                                   | Error                                                                                                                 | Cause                                                                                                                                                                                         | Resolved when                                                                                                    |
| ---------- | --------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| 2026-04-17 | `src/lib/layouts/AppShellLayout.svelte`                   | `This comparison appears to be unintentional… "/dashboard"` (type overlap)                                            | Route type union generated by `svelte-kit sync` is not picked up by VS Code TS server until the dev server starts. Type is correctly in `.svelte-kit/types/src/routes/$types.d.ts`.           | Run `dev-web.sh` once to trigger hot reload and TS server refresh                                                |
| 2026-04-18 | All `.svelte` files in `features/quant-pipeline` worktree | `Cannot find module '$app/environment'`, `interface is reserved`, `Cannot find name '$props'/$state/$effect/$derived` | `.svelte-kit/tsconfig.json` not yet generated in the worktree — VS Code extension falls back to plain-JS parsing. `svelte-check` after `svelte-kit sync` confirms 0 errors.                   | Run `pnpm install && npx svelte-kit sync` once in `apps/web/` of the worktree, or start the dev server via dashy |
| 2026-04-18 | `features/quant-pipeline/apps/web/tsconfig.json`          | `Cannot read file '.../.svelte-kit/tsconfig.json'`                                                                    | Dashy deletes `.svelte-kit/` on every start and recreates it via `svelte-kit sync`; VS Code TS server reads tsconfig before sync completes. Resolves automatically once the dev server is up. | Wait ~5 s after starting via dashy, or run `npx svelte-kit sync` in `apps/web/` of the worktree                  |
